...
while gameover == False:
  ...
  for each_player in players:
      ...
      roll2 = dice.rollD12()
      rollsum = roll1 + roll2
      each_player["rollsum"].append(rollsum)
      current_rolls.append(rollsum)
      print(f"Player {each_player["name"]} rolled a D8 and a D12, the result was {roll1} and {roll2}, the sum was {rollsum}")
  ...
  winners = []
  for each_player in players:
      if each_player["rollsum"][-1] == max_roll:
          each_player["wins"] += 1
...
with open(filename, "w") as file:
    ...
    for r in range(round):
        ...
        for i, each_player in enumerate(players):
            rolls_str += f"{each_player['name']} dice sum was {each_player['rollsum'][r]}"
            if i < len(players) - 1:
                rolls_str = ""
